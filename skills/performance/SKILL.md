...
```bash
/performance --cache-enable --size 1GB
/performance --cache-clear --older-than 7days
...
```bash
/performance --parallel --workers 4 .github/
/performance --batch-size 10 --timeout 300s
```bash

### Resource Optimization

```bash
/performance --optimize-memory --limit 2GB
/performance --balance-cpu --cores 8
```bash
